...
BaseType_t xReturn;
	...
	#else
	{
		...
		xReturn = xTaskCreate(	prvIdleTask,													/* 创建空闲任务，让CPU总能有事可做 */
	...
	#if ( configUSE_TIMERS == 1 )
	{
		if( xReturn == pdPASS )
		{
			xReturn = xTimerCreateTimerTask();													/* 创建软件定时器(任务形式) */
		}
		else
		{
			mtCOVERAGE_TEST_MARKER();
	...
	if( xReturn == pdPASS )
	{
		...
		xSchedulerRunning = pdTRUE;								/* 调度器启动标志位置位 */
		...
		portable interface. */
		if( xPortStartScheduler() != pdFALSE )	/* 配置SysTick、PendSV的中断优先级(最低优先级,优先级号为15)，并设置好SysTick定时器的周期（默认1ms）, 启动第一个任务 */
		{
